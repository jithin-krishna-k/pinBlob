...
const BLOB_API_URL = "https://3xz7o29l6rzizpy3.public.blob.vercel-storage.com"
...
export async function listBlobs(): Promise<BlobImage[]> {
    try {
        ...
        const response = await fetch(`${BLOB_API_URL}/list`, {
            method: "GET",
            headers: {
...
export async function uploadBlob(file: File): Promise<BlobImage> {
    try {
        ...
        const presignedResponse = await fetch(`${BLOB_API_URL}/presigned-upload`, {
            method: "POST",
            headers: {
                Authorization: `Bearer ${token}`,
                "Content-Type": "application/json",
